...
SELECT 
    ...
    CASE 
        WHEN (SELECT COUNT(*) FROM training_sessions WHERE is_current = true) = 1 THEN '✅ 正常'
        WHEN (SELECT COUNT(*) FROM training_sessions WHERE is_current = true) = 0 THEN '❌ 缺少当前期次'
        ELSE '⚠️ 多个当前期次'
    END as status
...
SELECT 
    'student_in_authorized' as item,
    (SELECT COUNT(*) FROM authorized_users WHERE email = 'student@test.com' AND role = 'student') as count_val,
    CASE 
        WHEN (SELECT COUNT(*) FROM authorized_users WHERE email = 'student@test.com' AND role = 'student') = 1 THEN '✅ 存在'
        WHEN (SELECT COUNT(*) FROM authorized_users WHERE email = 'student@test.com' AND role = 'student') = 0 THEN '❌ 缺失'
        ELSE '⚠️ 异常'
    END as status
...
SELECT 
    'student_in_profiles' as item,
    (SELECT COUNT(*) FROM profiles WHERE email = 'student@test.com' AND role = 'student') as count_val,
    CASE 
        WHEN (SELECT COUNT(*) FROM profiles WHERE email = 'student@test.com' AND role = 'student') = 1 THEN '✅ 存在'
        WHEN (SELECT COUNT(*) FROM profiles WHERE email = 'student@test.com' AND role = 'student') = 0 THEN '❌ 缺失'
        ELSE '⚠️ 异常'
    END as status
...
SELECT 
    'student_in_current_session' as item,
    (SELECT COUNT(*) 
     FROM session_students ss
     JOIN training_sessions ts ON ss.session_id = ts.id
     JOIN profiles p ON ss.user_id = p.id
     WHERE p.email = 'student@test.com' AND ts.is_current = true) as count_val,
    CASE 
        WHEN (SELECT COUNT(*) 
              FROM session_students ss
              JOIN training_sessions ts ON ss.session_id = ts.id
              JOIN profiles p ON ss.user_id = p.id
              WHERE p.email = 'student@test.com' AND ts.is_current = true) = 1 THEN '✅ 存在'
        WHEN (SELECT COUNT(*) 
              FROM session_students ss
              JOIN training_sessions ts ON ss.session_id = ts.id
              JOIN profiles p ON ss.user_id = p.id
              WHERE p.email = 'student@test.com' AND ts.is_current = true) = 0 THEN '❌ 缺失'
        ELSE '⚠️ 异常'
    END as status
ORDER BY item;
